...
func _handle_scores(scores: Dictionary) -> void:
	is_enabled = false
	...
	var max_score : int = scores.values().max()
	var max_players : Array = scores.keys().filter(func(p: int) -> bool: return scores[p] == max_score)
	...
	var player : BrawlerController = get_tree().get_nodes_in_group("Brawlers")\
		.filter(func(it: BrawlerController) -> bool: return it.player_id == player_id)\
		.pop_back()
