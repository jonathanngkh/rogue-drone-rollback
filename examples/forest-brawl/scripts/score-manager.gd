...
func _handle_spawn(brawler: BrawlerController) -> void:
	var id := brawler.player_id
	_scores[id] = _scores.get(id, 0)
	_brawlers[id] = brawler

func _handle_fall(brawler: BrawlerController) -> void:
	var id := brawler.player_id
...
func _handle_respawn(brawler: BrawlerController) -> void:
	# Hide scoreboard
	if brawler.player_id == multiplayer.get_unique_id():
		scorescreen.active = false

func _handle_despawn(brawler: BrawlerController) -> void:
	_scores.erase(brawler.player_id)
	_brawlers.erase(brawler.player_id)

@rpc("authority", "reliable", "call_remote")
func _submit_scores(scores: Dictionary) -> void:
...
func _render_scores() -> Dictionary:
	...
	for pid: int in _scores:
		var brawler : BrawlerController= _brawlers[pid]
