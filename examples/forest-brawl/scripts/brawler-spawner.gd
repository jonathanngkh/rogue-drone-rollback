...
func _ready() -> void:
	...
	initial_settings_received.connect(func() -> void: print('initial settings received signal'))
...
func _handle_stop() -> void:
	...
	for avatar: DroneController in avatars.values():
		avatar.queue_free()
	avatars.clear()


func _spawn(id: int) -> DroneController:
	var avatar : DroneController = player_scene.instantiate()
...
@rpc("any_peer", "reliable", "call_local")
func initial_settings_sent() -> void:
