...
func host_only() -> void:
	...
	if brawler_spawner != null:
		...
		brawler_spawner.set_spawn_host_avatar.rpc(false)
	host()
...
func _handle_connect(address: String, port: int) -> Error:
	...
	if role == Role.CLIENT:
		...
		await Async.condition(
			func()  -> bool: return peer.get_connection_status() != MultiplayerPeer.CONNECTION_CONNECTING
		)
